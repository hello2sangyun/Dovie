...
class ChatManager: ObservableObject {
    ...
    func loadChatRooms() {
        ...
        .sink(
            ...
            receiveValue: { (chatRooms: [ChatRoom]) in
    ...
    func sendMessage(content: String) {
        ...
        .sink(
            receiveCompletion: { (completion: Subscribers.Completion<Error>) in },
            receiveValue: { (message: Message) in
    ...
    private func loadUnreadCounts() {
        ...
        .sink(
            receiveCompletion: { (completion: Subscribers.Completion<Error>) in },
            receiveValue: { (counts: [String: Int]) in
                // String 키를 Int로 변환
                var intCounts: [Int: Int] = [:]
                for (key, value) in counts {
                    if let intKey = Int(key) {
                        intCounts[intKey] = value
                    }
                }
                self.unreadCounts = intCounts
            }
        )
        .store(in: &cancellables)
    }
