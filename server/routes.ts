...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/chat-rooms/:chatRoomId/messages", async (req, res) => {
    try {
      ...
      const messages = await storage.getMessages(chatRoomId, limit, offset);
  ...
  app.post("/api/chat-rooms/:chatRoomId/messages", async (req: any, res: any) => {
    const userId = req.headers["x-user-id"];
    if (!userId) {
      return res.status(401).json({ message: "Not authenticated" });
    }
    ...
    try {
      ...
      // Telegram/WhatsApp-style intelligent push notification logic
      try {
        ...
        console.log(`🔔 Participants: ${chatRoom?.participants?.length || 0}`);
        ...
        if (chatRoom?.participants) {
          ...
          console.log(`🔔 Sender: ${sender?.displayName || 'undefined'}`);
          console.log(`🔔 Recipients: ${recipients.length}`);
          
          // Filter conditions based on Telegram/WhatsApp behavior
          const isSystemMessage = messageData.isSystemMessage || false;
          const isValidSender = !!(sender?.id && sender?.displayName);
          const isAutoGeneratedContent = messageData.content && (
            messageData.content.includes('🎬 YouTube 동영상') || 
            messageData.content.includes('유튜브 검색') ||
            messageData.content.includes('⏰ 리마인더') ||
            messageData.content.startsWith('시스템:')
          );
          
          console.log(`🔔 Filter Check:`);
          console.log(`   - isSystemMessage: ${isSystemMessage}`);
          console.log(`   - isValidSender: ${isValidSender}`);
          console.log(`   - isAutoGeneratedContent: ${isAutoGeneratedContent}`);
          console.log(`   - WILL SEND PUSH: ${!isSystemMessage && isValidSender && !isAutoGeneratedContent}`);
          
          // Only send notifications for real user messages (like Telegram/WhatsApp)
          if (!isSystemMessage && isValidSender && !isAutoGeneratedContent) {
            ...
            console.log(`📱 Suppressed notification - System: ${isSystemMessage}, Valid sender: ${isValidSender}, Auto-generated: ${isAutoGeneratedContent}`);
          }
        }
      } catch (pushError) {
  ...
  // 수정된 iOS 프로젝트 다운로드 페이지
  app.get("/ios-download-fixed", (req, res) => {
    const downloadPage = `
<!DOCTYPE html>
<html lang="ko">
<head>
...
<body>
  ...
  app.get("/ios-fixed-download", (req, res) => {
    const filePath = path.join(process.cwd(), 'ios', 'dovie-messenger-ios-final.zip');
...
<body>
  ...
  // iOS 서버 연결형 프로젝트 다운로드
  app.get("/ios-download-server", (req, res) => {
    const filePath = path.join(process.cwd(), "public/dovie-messenger-ios-server-connected.zip");
    ...
    res.set({
      'Content-Type': 'application/zip',
      'Content-Disposition': 'attachment; filename="dovie-messenger-ios-server-connected.zip"',
      'Cache-Control': 'no-cache'
    });
  ...
  app.get('/api/bookmarks', async (req, res) => {
    ...
    try {
      ...
      const getMimeTypeFromFileName = (fileName: string | null): string | null => {
      ...
      const bookmarksWithMessages = await Promise.all(
        userBookmarks.map(async (bookmark) => {
          ...
          if (message && message.fileName) {
            // Add fileType based on fileName
            return {
              ...bookmark,
              message: {
                ...message,
                fileType: getMimeTypeFromFileName(message.fileName)
              }
            };
          }
          return {
