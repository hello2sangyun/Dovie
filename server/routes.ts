...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/hashtags/search", async (req, res) => {
    try {
      ...
      const hashtags = await storage.searchUserHashtags(userId, query);
      res.json({ hashtags });
    } catch (error) {
  ...
  app.get("/api/hashtags/suggestions", async (req, res) => {
    try {
      ...
      const suggestions = await storage.getUserHashtagSuggestions(userId, prefix);
      res.json({ suggestions });
    } catch (error) {
  ...
  app.get("/api/files/search", async (req, res) => {
    try {
      ...
      const hashtagsParam = req.query.hashtags as string;
      if (!hashtagsParam) {
        return res.status(400).json({ message: "Hashtags parameter is required" });
      }

      const hashtags = hashtagsParam.split(',').map(tag => tag.trim()).filter(Boolean);
      const files = await storage.searchFilesByHashtags(userId, hashtags);
      res.json({ files });
    } catch (error) {
      console.error("Search files by hashtags error:", error);
      res.status(500).json({ message: "Failed to search files" });
    }
  });

  app.post("/api/files/:fileId/hashtags", async (req, res) => {
    try {
      const userId = req.session?.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Authentication required" });
      ...
      for (const hashtagText of hashtags) {
        ...
        if (cleanTag) {
          ...
          const hashtag = await storage.createOrUpdateHashtag(cleanTag, userId);
          
          // Link hashtag to file
          await storage.addFileHashtag(fileId, hashtag.id, userId);
          
          // Update user hashtag index
          await storage.updateUserHashtagIndex(userId, hashtag.id);
        }
      }

      res.json({ success: true });
    } catch (error) {
      console.error("Add file hashtags error:", error);
      res.status(500).json({ message: "Failed to add hashtags" });
    }
  });

  app.get("/api/hashtags/index", async (req, res) => {
    try {
      const userId = req.session?.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Authentication required" });
      ...
      const index = await storage.getUserHashtagIndex(userId);
      res.json({ index });
    } catch (error) {
      console.error("Get hashtag index error:", error);
      res.status(500).json({ message: "Failed to get hashtag index" });
    }
  });
