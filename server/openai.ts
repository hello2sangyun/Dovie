...
📋 분석 규칙 (높은 민감도 - 더 많은 메시지를 감지):
- **무시할 것**: 단순 인사말만 무시 ("안녕", "뭐해?")
- **감지할 것**: 
  - 제안/질문 형태도 감지 ("점심 뭐 먹을까?", "언제 만날까?", "어디서 볼까?")
  - 암묵적 약속/일정 ("오늘 저녁?", "주말에 뭐해?", "내일 시간 돼?")
  - 일상적 할 일도 감지 ("장 봐야해", "운동 가야지", "청소해야 돼")
  - 감탄사가 포함되어도 실질적 내용이 있으면 감지 ("ㅋㅋㅋ 내일 3시에 보자")
...
- "다음주"는 7일 후부터 시작
- "~해야지", "~할게", "~해야 돼", "~할까?", "~어때?" 등의 표현 주목
- 의문문도 일정/약속 의미가 있으면 감지 ("내일 저녁 괜찮아?", "점심 먹을래?")
- 부정문("안 가", "안 해")은 무시
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `당신은 음성 인식 텍스트를 최소한으로 보정하는 AI입니다. 원문의 톤과 말투를 절대적으로 보존하세요.

⚠️ 핵심 원칙: 원문을 최대한 그대로 유지하고, 정말 명백한 오류만 수정하세요.

절대 수정하지 마세요:
- 띄어쓰기 (그대로 유지)
- 맞춤법 (그대로 유지)
- 말투와 톤 (반말, 존댓말, 느낌표, 물음표 등 절대 변경 금지)
- 억양이나 뉘앙스 (그대로 유지)
- 구어체 표현 ("어", "음", "그", "저" 등 그대로 유지)

수정 가능한 경우 (매우 제한적):
- 명백하게 잘못 인식된 단어만 수정 (예: 대화 맥락상 100% 확실한 경우)
- 예시: 채팅에서 계속 "수진"이라는 이름이 나왔는데 음성 인식이 "수정"으로 잘못 인식한 경우

의심스러우면: 원문을 그대로 반환하세요.

수정된 텍스트만 반환하세요. 설명이나 부가 정보는 포함하지 마세요.`
        },
        {
          role: "user",
          content: `최근 대화 맥락:\n${recentContext}${userStyleContext}\n\n음성 인식 텍스트:\n${transcription}\n\n위 텍스트에서 명백한 오류만 수정하고, 의심스러우면 원문 그대로 반환하세요.`
        }
      ],
      max_tokens: 300,
      temperature: 0.1
