...
- June 24, 2025: Fixed duplicate push notification issue and optimized mobile PWA system:
  - Resolved duplicate push notifications by ensuring only one subscription per user device
  - Enhanced upsertPushSubscription to replace all existing subscriptions instead of just matching endpoints
  - Added initialization check to prevent duplicate subscription registrations from multiple components
  - Optimized database cleanup to eliminate multiple subscriptions causing duplicate notifications
  - Push notifications now send exactly once per message to each user device
