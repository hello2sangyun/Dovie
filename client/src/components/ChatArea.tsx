...
export default function ChatArea({ chatRoomId, onCreateCommand, showMobileHeader, onBackClick }: ChatAreaProps) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].size > maxSize) {
        alert(`${files[i].name} 파일이 500MB를 초과합니다. 각 파일은 500MB 이하여야 합니다.`);
        return;
      }
    }
  ...
  return (
    <div 
      ...
      data-chat-area="true"
      className={cn(
        "h-full flex flex-col relative mb-0 pb-0 overflow-hidden",
              ...
              <div key={msg.id}>
                ...
                <div className={cn("flex w-full", isMe ? "justify-end" : "justify-start")}>
                  <div 
                    ...
                    className={cn(
                      "flex items-end mb-1 transition-all duration-500 group max-w-[92%]",
